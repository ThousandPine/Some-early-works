...
int main()
{
	//玩家
		...
		//玩家基本属性:
					...
					int p_hp = 1000, p_strength = 100, p_dodge = 10, p_critical = 20, p_luck = 0, p_VIT = 100, \
						...
						p_attack_range_short = 100, p_attack_range_long = 0, p_speed = 100, p_magic = 100;	
					//  力量基准值						暴击率基准值
					int p_strength_origin = p_strength, p_critical_origin = p_critical;
		...
		//效果:	
			...
			int timer[timer_n], weapon_long_mod = 0, weapon_short_mod = 0, buff_short_damage = 0, buff_short_range = 0, buff_short_critical = 0, buff_magic = 0, \
				...
				buff_long_damage = 0, buff_long_range = 0, buff_long_critical = 0, weapon_special_judge_0 = 30, weapon_special_judge_1 = 200, enchantment_mod = 0, weapon_magic_mod = 0, \
	...
	//BOSS
		//BOSS基本属性:
				...
				int b_hp = 10000, b_strength = 60, b_dodge = 20, b_critical = 20, b_attack_range = 100, b_speed = 100;
	...
	menue0:
				...
				if (p_attack_range_short + buff_short_range >= range)				//近战
				{
					...
					if (damage)	//附魔效果
					{
						...
						else if (enchantment_mod == 2)	//附魔效果:一鼓作气
						{
							...
							if (enchatment_judge_0 > 80)enchatment_judge_1 -= 10;
						}
						printf("造成了%d伤害\n", damage);
					}
				...
				do {
					...
					case '1':	//选项--2.1查看
						...
						printf("玩家:\n\n力量:%d  魔法:%d%%   暴击率(近战):%d%%   暴击率(远程):%d%%\n\n攻击距离(近战):%d   攻击距离(远程):%d   闪避率:%d%%   移动速度:%d\n\n\n"\
							, p_strength_origin, p_magic+buff_magic, p_critical_origin + buff_short_critical, p_critical_origin + buff_long_critical, p_attack_range_short, p_attack_range_long, p_dodge, p_speed);
						printf("=v=:\n\n生命值:%d   攻击力:%d  暴击率:%d%%  闪避率:%d%%\n\n攻击距离:%d  移动速度:%d\n"\
							, b_hp,b_strength,b_critical,b_dodge,b_attack_range,b_speed);
						display(1, 20);
						getch();
						if (weapon_magic_mod == 1)goto menue2;
						break;
					case '2':	//选项--2.2探索
						if (p_VIT >= 30||timer[1])
						{
							...
							do {
								...
								case '1':		//选项--2.2.1.货舱
									...
									if (judge <= 85)//收获
									{
										...
										{
											judge = rand() % 100 + 1;
											if (judge <= 25) { prop[0]++; printf("生命药水\n"); }
											else if (judge <= 45) { prop[1]++; printf("体力药水\n"); }
											else if (judge <= 55) { prop[2]++; printf("昏睡红茶\n"); }
											else if (judge <= 60) { prop[3]++; printf("命运骰子\n"); }
										...
										{
											judge = rand() % 100 + 1;
											if (judge <= 40) { levelbook[0]++; printf("《芝士就是_ _!》\n"); }
								...
								case '2':		//选项--2.2.2.军备库 
									...
									if (judge <= 70)//收获
									{
										...
										{
											judge = rand() % 100 + 1;
											if (judge <= 20) { prop[0]++; printf("生命药水\n"); }
											else if (judge <= 45) { prop[1]++; printf("体力药水\n"); }
											else if (judge <= 70) { prop[4]++; printf("肾上腺素\n"); }
								...
								case '3':		//选项--2.2.3.典藏馆
									...
									if (judge <= 70)//收获
									{
										...
										{
											judge = rand() % 100 + 1;
											if (judge <= 50) { prop[0]++; printf("昏睡红茶\n"); }
											else if (judge <= 100) { prop[1]++; printf("命运骰子\n"); }
										}
						...
						do {
							...
							case '3':		//选项--4.3.3.守护者之杖
								if (weapon_magic[in - 49])
								{
									display(0, 50);
									display(1, 20);
									printf("守护者之杖\n\n魔法+20%%\n\n*屹立:回合结束后生命值+%.lf 体力值+%.lf\n\n*救主灵刃:生命值少于50时恢复到150(仅一次)\n\n\nX.装备 Z.返回\n",10.0*(p_magic+buff_magic)/100,20.0*(p_magic+buff_magic)/100);
									display(1, 20);
									do {
										back = 0;
										in = getch();
		...
		if (p_hp > 0 && b_hp > 0)	//结算效果
		{
			...
			if (timer[5]) {timer[5]--; printf("毒雾:造成%.lf点伤害\n", \
				0.05 * (p_magic + buff_magic) / 100); b_hp -= b_hp * 0.05 * (p_magic + buff_magic) / 100;}		//法术:毒雾
			if (timer[6]) { timer[6]--; printf("*剧毒:造成60点伤害\n"); b_hp -= 60; }							//武器:淬毒匕首
			...
			if (weapon_magic_mod == 3) { printf("*屹立:生命值+%.lf  体力+%.lf\n", \
				10.0 * (p_magic + buff_magic) / 100, 20.0 * (p_magic + buff_magic) / 100); p_hp += 10.0 * (p_magic + buff_magic) / 100, p_VIT += 20.0 * (p_magic + buff_magic) / 100; }	//守护者之杖:屹立
			if (weapon_magic_mod == 3 && p_hp < 50 && k) { p_hp = 150; k = 0; printf("*触发救主灵刃,生命值恢复至150\n"); }	//守护者之杖:救主灵刃
			display(1, 20);
			getch();
		}
		...
		if (p_hp > 0 && b_hp > 0)	//BOSS回合
		{
			...
			{
				if (p_hp >= b_hp)	//特殊技能:背水一战
				{
					printf("*=v=发动了特殊技能:背水一战\n\n\n");
					printf("生命值%+d  攻击力%+d  移动速度%+.lf  闪避率+%d%% 攻击距离%+d\n",p_hp*5-b_hp,p_strength*1.5-b_strength,p_speed*0.7-b_speed,50,p_attack_range_short/2-b_attack_range);
					b_hp = 5 * p_hp;
					b_strength = 1.5 * p_strength;
					b_speed = p_speed * 0.7;
					b_dodge += 50;
					b_attack_range = p_attack_range_short / 2;
				}
				else if (b_hp <= 2 * p_hp || b_hp >=3 * p_hp)	//进攻
				{
