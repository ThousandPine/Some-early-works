			...
			menue4:
				...
				do {
					...
					menue4_3:
						...
						else if (weapon_magic_mod == 1)printf("支援法杖\n");
						else if (weapon_magic_mod == 1)printf("守护者之杖\n");
						printf("\nZ.返回\n");
						display(1, 20);
						do {
							back = 0;
		...
		if (p_hp > 0 && b_hp > 0)	//BOSS回合
		{
			...
			else	//BOSS行动
			{
				if (boss_special&&p_hp >= b_hp)	//特殊技能:背水一战
				{
					...
					printf("生命值%+d  攻击力%+.lf  移动速度%+.lf  闪避率+%d%% 攻击距离%+d\n",p_hp*5-b_hp,b_strength*1.5-b_strength,60-b_speed,30,b_attack_range/2-b_attack_range);
					b_hp = 5 * p_hp;
					b_strength = 1.5 * b_strength;
					b_speed = 60;
					b_dodge += 30;
					b_attack_range = b_attack_range / 2;
				}
